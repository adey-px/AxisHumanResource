...
def notification():
    if request.method == "POST":
        today = date.today()
        sender = mongo.db.users.find_one({"email": session["user"]})["email"]
        notice = {
...
def change_password():
    if request.method == "POST":
        ...
        user_password = mongo.db.users.find_one({"email": session["user"]})["password"]

        # Compare user password in db with value in form input as old password
        match_password = check_password_hash(user_password, request.form.get("oldpass"))
...
# Employee message api/route in dashboard page
...
def emp_message(email):
    ...
    user_email = mongo.db.users.find_one({"email": session["user"]})["email"]

    user_name = mongo.db.users.find_one({"email": session["user"]})["full_name"]
...
def edit_department(item_id):
    if request.method == "POST":
        ...
        edit = {"department": request.form.get("department_name")}
